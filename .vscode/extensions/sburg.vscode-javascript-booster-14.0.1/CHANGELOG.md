...
-   Add new refactoring: `Replace ?: with if-else` (#19):
    ...
    ![Replace ?: with if-else](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/replace-ternary-with-if-else.gif)
...
-   Add new range-based code action: `Call await statements in parallel with Promise.All()` (#7):
    ...
    ![Call await statements in parallel with Promise.All()](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/convert-await-sequence-into-parallel.gif)

-   Add new option to adjust formatting for generated code (only needed when a code action generates new code, the formatting of the unaffected code is always preserved). See `javascriptBooster.formattingOptions` and [#10](https://github.com/xsburg/vscode-javascript-booster/issues/10).
...
-   Fixed `replace if-else with ?:` refactoring, now supports refactoring of pure expressions:
    ...
    ![replace if-else with ?:](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/replace-if-else-with-ternary_expression-statement.gif)

### 0.10.0

-   [VSCode API] Switched to using selection parameter passed into `provideCodeActions()` (Fixes [#5](https://github.com/xsburg/vscode-javascript-booster/issues/5))
...
-   `Remove redundant else` now supports the case when `if` branch ends with return statement:
    ...
    ![](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/remove-redundant-else_return-in-if.gif)

-   `Replace with ternary` can now replace conditional return statements:

    ![](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/replace-if-else-with-ternary_return.gif)

-   Added new action: `Simplify if-else`.

    ![](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/simplify-if-else.gif)

-   Added new action: `Simplify ?:`.

    ![](https://github.com/xsburg/vscode-javascript-booster/raw/master/resources/simplify-ternary.gif)
...
-   Fixed [#1](https://github.com/xsburg/vscode-javascript-booster/issues/1): Sequence of string literals doesn't convert properly when transforming to template literal.
