...
class Manager extends dispose_1.Disposable {
     ...
     */
    async openPreviewAtFileString(filePath, previewType, ignoreFileRoot = false) {
        if (filePath === '') {
            return this._openPreviewWithNoTarget();
        }
        const workspace = await pathUtil_1.PathUtil.GetWorkspaceFromRelativePath(filePath, ignoreFileRoot);
        if (workspace) {
